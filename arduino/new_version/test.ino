...
const char* url = "http://192.168.33.39:8005"; // base URL
...
const int record_buffer_size = 10;
String record_buffer[record_buffer_size];
int record_buffer_count = 0;
...
unsigned long config_pull_millis_now = 0, config_pull_millis_old = 0;
const unsigned long config_pull_interval = 5 * 60 * 1000; // 5 minutes
...
void store_data_to_sd_card() {
  ...
  // Buffer the record
  record_buffer[record_buffer_count++] = record_data;

  // Check if need to flush buffer (date change or buffer full)
  String current_date = rtc_get_date();
  bool date_changed = (last_record_date != current_date);
  if (date_changed) {
    last_record_date = current_date;
  }

  if (record_buffer_count >= record_buffer_size || date_changed) {
    // Open files
    String record_filename = "/record_" + current_date + ".txt";
    record_file = SD.open(record_filename.c_str(), FILE_APPEND);
    if (!record_file) {
      Serial.println("File not found or failed to open : record_file");
      record_file = SD.open(record_filename.c_str(), FILE_WRITE);
      if (!record_file) {
    ...
    for (int i = 0; i < record_buffer_count; i++) {
      sd_result = record_file.println(record_buffer[i].c_str());
      if (sd_result == 0) {
        Serial.println("file : " + record_filename + " | write fail!");
      }
    }
    record_file.flush();
    record_file.close();

    // Write to offline file if needed
    if (avialable_wifi == false) {
      ...
      for (int i = 0; i < record_buffer_count; i++) {
        sd_result = record_offline_file.println(record_buffer[i].c_str());
        if (sd_result == 0) {
    ...
    // Write alarms for each record in buffer (if any)
    String alarm_filename = "/alarm_" + current_date + ".txt";
    alarm_file = SD.open(alarm_filename.c_str(), FILE_APPEND);
    if (!alarm_file) {
      Serial.println("File not found or failed to open : alarm_file");
      alarm_file = SD.open(alarm_filename.c_str(), FILE_WRITE);
      if (!alarm_file) {
    ...
    for (uint8_t i = 0; i < sizeof(mcp) / sizeof(mcp[0]); i++) {
      if (mcp9600_alarm[i] == true) {
        ...
        if (sd_result == 0) {
          Serial.println("file : " + alarm_filename + " | write fail!");
        }
      }
    }
    alarm_file.flush();
...
void store_data_to_server() {
  http.begin(String(url) + "/record/add");
...
void pull_and_apply_config() {
  if (WiFi.status() != WL_CONNECTED) return;

  HTTPClient config_http;
  config_http.begin(String(url) + "/machine/get");
  config_http.setTimeout(http_timeout);
  config_http.addHeader("Content-Type", "application/x-www-form-urlencoded");

  String post_data = "machine_name=" + String(host_name);
  int http_response_code = config_http.POST(post_data);

  if (http_response_code > 0) {
    String payload = config_http.getString();
    // Parse JSON
    StaticJsonDocument<1024> doc;
    DeserializationError error = deserializeJson(doc, payload);
    if (!error && doc.is<JsonArray>()) {
      JsonArray arr = doc.as<JsonArray>();
      if (arr.size() > 0 && arr[0].containsKey("data")) {
        JsonArray dataArr = arr[0]["data"].as<JsonArray>();
<<<<<<< Updated upstream
        for (JsonObject item : dataArr) {
          if (!item.containsKey("name")) continue;
          String data_name_from_server = item["name"].as<String>();
          for (uint8_t i = 0; i < sizeof(mcp9600_data_name)/sizeof(mcp9600_data_name[0]); i++) {
            if (mcp9600_data_name[i] == data_name_from_server) {
              if (item.containsKey("max")) mcp9600_max_limit[i] = item["max"].as<float>();
              if (item.containsKey("min")) mcp9600_min_limit[i] = item["min"].as<float>();
              // Optionally: handle line_color if you want to use it
              break;
            }
          }
=======
        for (uint8_t i = 0; i < dataArr.size() && i < (sizeof(mcp9600_data_name)/sizeof(mcp9600_data_name[0])); i++) {
          if (dataArr[i].containsKey("name")) mcp9600_data_name[i] = dataArr[i]["name"].as<String>();
          if (dataArr[i].containsKey("max")) mcp9600_max_limit[i] = dataArr[i]["max"].as<float>();
          if (dataArr[i].containsKey("min")) mcp9600_min_limit[i] = dataArr[i]["min"].as<float>();
          // Optionally: handle line_color if you want to use it
>>>>>>> Stashed changes
        }
      }
    ...
    Serial.println("Config pull failed: " + String(http_response_code));
  }
  config_http.end();
}
...
void core_0(void *not_use) {
  while (true) {
    ...
    // --- Pull config every 5 minutes ---
    config_pull_millis_now = millis();
    if (config_pull_millis_now - config_pull_millis_old >= config_pull_interval) {
      pull_and_apply_config();
      config_pull_millis_old = config_pull_millis_now;
    }
...
void setup() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    // --- Pull config after WiFi connected ---
    pull_and_apply_config();
    config_pull_millis_old = millis();
  }
  ...
  //pcf8563
  rtc.begin();
    rtc.setDateTime(2025, 4, 19, 23, 55, 00);
  //
  //  while(true) {
  //    delay(100);
  //  }
