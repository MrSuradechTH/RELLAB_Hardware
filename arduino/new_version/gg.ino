...
const char* url = "http://192.168.254.39:8005/record/add";
...
int count_line(const char * path) {
  ...
  if (myFile) {
    const int bufferSize = 128; // Adjust buffer size as needed
...
void store_data_from_sd_card_to_server() {
  ...
  if (SD.exists("/data.txt")) {
    ...
    if (dataFile && uploadFile) {
      ...
      dataFile.close();
      uploadFile.close();
      SD.remove("/data.txt");  // Delete data.txt after merging
      Serial.println("Done merging data files");
    } else {
      Serial.println("Error merging data files");
      return;
    }
  }

  if (!SD.exists("/upload_data.txt")) {
    Serial.println("No upload data available");
    return;
  }

  // Process each line from upload_data.txt
  while (count_line("/upload_data.txt") > 0) {
    Serial.println("Uploading data...");

    // Read first line using the custom read_line function
    String raw_data = read_line("/upload_data.txt", 0);  // Read line 0
    Serial.println("count line is : " + String(count_line("/data.txt")));
    
    if (count_line("/upload_data.txt") > 0) {
      // Convert raw data to correct format for HTTP request
      String json_data = "data=" + raw_data;
      ...
      if (http_response_code > 0) {
        Serial.println("Uploaded: " + raw_data);
        remove_line("/upload_data.txt", 0);  // Remove first line after successful upload
      } else {
        Serial.println("Upload failed, keeping data for retry");
        break;  // Stop uploading if there's an issue
      }
...
void core_0(void *not_use) {
  while(true) {
    ...
    if (ESP.getFreeHeap() < 50000 || working_time_millis_now - working_time_millis_old >= 3600000) {
      ESP.restart();
    }
    ...
    if (sd_card_millis_now - sd_card_millis_old >= 1000 && avialable_wifi == false) {
